...
| 3 — ESP32 ↔ Uno UART link | ⚠️ Blocked — voltage mismatch found | Real 64-feature JSON payload implemented, but hardware link currently non-functional. Root cause identified (5V/3.3V mismatch, no level shifting on Uno TX → ESP32 RX) and fix designed — see §4.2. **Not yet verified on hardware.** |
...
**⚠️ Voltage level mismatch — a level shifter is required on one leg of this link.** The Uno is 5V logic; the ESP32-S3 is 3.3V logic and its GPIOs are **not 5V tolerant**. Feeding the Uno's 5V TX output directly into an ESP32 RX pin doesn't reliably register as a clean logic HIGH, and repeated exposure can degrade the pin over time.

| Uno R3 pin | ESP32-S3-CAM pin | Signal | Level shifting needed? |
|---|---|---|---|
| D2 (SoftwareSerial RX) | GPIO 39 (UART TX) | ESP32-S3-CAM → Uno | **No** — ESP32's 3.3V output registers as a valid HIGH on the Uno's 5V-logic input (Uno's HIGH threshold is ~3V), so this leg is safe to wire direct. |
| D3 (SoftwareSerial TX) | GPIO 38 (UART RX) | Uno → ESP32-S3-CAM | **Yes** — see voltage divider below. Do not wire this leg direct. |
| GND | GND | Common ground — do not skip this | — |

**Voltage divider for Uno TX (D3) → ESP32 RX (GPIO 38):**
```
Uno D3 (TX, 5V) ──[ 1kΩ ]──┬──[ 2kΩ ]── GND
                            │
                       ESP32 RX (GPIO 38)
```
This divides 5V down to `5V × (2k/(1k+2k)) ≈ 3.33V` — safely within the ESP32's input range.
...
**Debugging note (found via logic analyzer, fix not yet verified on hardware):** the link initially appeared completely dead — both TX lines sitting idle-high with no traffic seen at all. A standalone UART pin-isolation test sketch (send-only, no other code running) confirmed both boards' TX lines were actually toggling correctly. The ESP32 simply never received anything, on multiple different GPIOs tried, which pointed away from "dead pin" and toward a signaling-level problem — i.e. this voltage mismatch, undocumented in an earlier version of this doc. The divider above is the fix; needs to be tested on real hardware before this note is removed.
